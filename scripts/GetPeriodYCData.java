...
public class GetPeriodYCData
{
   ...
   public static void main(String args[]) throws Exception {
     ...
     String timeStart = Integer.toString(keyInputObj.getIntValue("timetag0"));//整型时间戳转字符串
     String timeEnd = Integer.toString(keyInputObj.getIntValue("timetag1"));//整型时间戳转字符串
     ...
     for(int i=0;i<dataLength;i++){
       List<Integer> listkey = Jedis.lrange((strings.get(i)+"_"+Integer.toString(integers.get(i))),-1,0);
       j=0;
       for (int k=0;k<listkey.size();k++){
         if (timeStart<=listkey.get(i)<=timeEnd){
          j++;
         }
       }
       Num[i]=j;
     ...
     /*for(int i=0;i<dataLength;i++){
      membersList[i] = jedis.get(strings.get(i)+"_"+Integer.toString(integers.get(i))+"_"+time);
      logger.info("The finding key is:" + (strings.get(i)+"_"+Integer.toString(integers.get(i))+"_"+time));
